...
--Problem 5
SELECT P.personId, P.name, C.title FROM People P
JOIN InvolvedIn I ON P.personId = I.personId
JOIN Cases C ON I.CaseId = C.CaseId
JOIN Locations L ON L.locationId = C.locationId
WHERE I.isCulprit = true AND C.locationId = P.locationId;


--Problem 6
SELECT P.personId, P.name, G.gender FROM Genders G
...
SELECT P.personId, P.name, Pr.description FROM Professions Pr
...
WHERE C.isclosed = false AND Pr.description = 'therapist';

--Problem 8 Not finished
SELECT A.codename, G.gender, P.password FROM Agents A
JOIN Genders G ON G.GenderID = A.GenderID
JOIN Passwords P ON P.AgentID = A.AgentID
WHERE P.password LIKE '%' + A.codename +'%';
